...
fn typed_scene_and_performance_macros_match_hardware_readback() {
    let bytes = fixture("kit-work-buffer-macros.syx");
    let raw = RawSysexObject::from_sysex(&bytes).unwrap();
...
fn macro_definition_fixture_preserves_unrelated_kit_bytes() {
    ...
    let baseline = RawSysexObject::from_sysex(&fixture("kit-work-buffer.syx"))
        .unwrap()
        .decoded_bytes()
        .unwrap();
    let defined = RawSysexObject::from_sysex(&fixture("kit-work-buffer-macros.syx"))
        .unwrap()
        .decoded_bytes()
        .unwrap();
