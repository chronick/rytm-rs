...
- Song: controlled name, rows, pattern chain, repeats, and 12-track mute masks were written to the
  work buffer, read back through the typed codec, and rolled back to an exact baseline.
...
The macro and Song certification JSON files record write/readback/rollback evidence. Their
corresponding baseline, defined, and restored SysEx files contain no sample audio.
