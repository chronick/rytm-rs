...
//!
//! The decoded Song object is 1,304 bytes. The layout below was confirmed against a connected
//! Analog Rytm MKII on OS 1.72 and cross-checked with the public [`midiator`] Song parser; the
//! user-visible semantics follow the [Analog Rytm MKII User Manual, OS 1.72].
//!
//! # Confirmed layout
//!
...
//!
//! The Song name, up to 64 rows, up to 256 total pattern positions shared by those rows, per-row
//! repeats, pattern chains, and the 12 drum-track mute bits at each pattern position.
//!
//! # What is preserved untouched
//!
//! Every mutation leaves the unidentified bytes exactly as they were decoded: the unidentified
//! header bytes, the two unidentified bytes in each row record, the one unidentified byte in each
//! pattern-position record, and the upper four bits of each mute word. A decode followed by an
//! encode is therefore byte-exact.
//!
//! # What is deliberately not modelled
//!
//! The OS 1.72 Song UI does not expose tempo overrides, per-row pattern-length overrides, jumps,
//! loops, row labels, or an explicit end row; playback ends after the final active row. Those
//! features are not inferred from the unidentified bytes and are not represented here.
//!
...
impl SongPattern {
    ...
    pub fn try_new(pattern: usize) -> Result<Self, RytmError> {
        ...
        Ok(Self {
            pattern: u8::try_from(pattern).map_err(|_| range_error(pattern, "song pattern"))?,
            mute_word: 0,
            unknown_flags: 0,
        })
    }
    ...
    pub fn set_pattern(&mut self, pattern: usize) -> Result<(), RytmError> {
        validate_range(pattern, "song pattern", 0, 127)?;
        self.pattern = u8::try_from(pattern).map_err(|_| range_error(pattern, "song pattern"))?;
        Ok(())
    }
...
impl SongRow {
    ...
    pub fn try_new(patterns: Vec<SongPattern>, repeats: usize) -> Result<Self, RytmError> {
        ...
        Ok(Self {
            patterns,
            repeats: u16::try_from(repeats)
                .map_err(|_| range_error(repeats, "song row repeats"))?,
            unknown_prefix: 0,
            unknown_flags: 0,
        })
    }
    ...
    pub fn set_repeats(&mut self, repeats: usize) -> Result<(), RytmError> {
        validate_range(repeats, "song row repeats", 1, 256)?;
        self.repeats =
            u16::try_from(repeats).map_err(|_| range_error(repeats, "song row repeats"))?;
        Ok(())
    }
...
impl Song {
    ...
    /// Creates an empty work-buffer Song with an explicit device ID.
    ...
    const fn empty(sysex_meta: SysexMeta) -> Self {
        let mut raw = [0; SONG_RAW_SIZE];
        // Observed in every Song the device has produced; meaning unidentified.
        raw[0x15] = 1;
        Self { sysex_meta, raw }
    }
    ...
    pub fn replace_rows(&mut self, rows: &[SongRow]) -> Result<(), RytmError> {
        ...
        for (row_index, row) in rows.iter().enumerate() {
            ...
            self.raw[offset + 1] = u8::try_from(row.patterns.len())
                .map_err(|_| range_error(row.patterns.len(), "song row pattern count"))?;
            self.raw[offset + 2] = row.unknown_flags;
            self.raw[offset + 3] = u8::try_from(row.repeats - 1)
                .map_err(|_| range_error(usize::from(row.repeats), "song row repeats"))?;
...
fn validate_range(
    ...
    if value < minimum || value > maximum {
        return Err(range_error(value, parameter_name));
    }
    Ok(())
}

fn range_error(value: usize, parameter_name: &str) -> RytmError {
    ParameterError::Range {
...
mod tests {
    ...
    fn rows_repeats_patterns_and_mutes_round_trip() {
        ...
        song.set_name("ROUND TRIP").unwrap();
    ...
    #[test]
    fn out_of_range_stored_song_index_is_rejected() {
        // Header layout is `F0 00 20 3C 07 <dev_id> <dump_id> <ver_hi> <ver_lo> <obj_nr>`,
        // so `obj_nr` is byte 9. The response checksum covers bytes 10..trailer only,
        // which leaves the header — and byte 9 with it — outside the checksummed region.
        const OBJ_NR_BYTE_INDEX: usize = 9;

        let song = Song::try_default(15).unwrap();
        let encoded = song.as_sysex().unwrap();
        assert_eq!(encoded[OBJ_NR_BYTE_INDEX], 15);

        let mut project = crate::RytmProject::try_default().unwrap();
        project.update_from_sysex_response(&encoded).unwrap();
        assert_eq!(project.songs()[15].index(), 15);

        // `libanalogrytm` does not range-check `obj_nr` while decoding, so a device
        // reporting a stored song past the last slot must be rejected, not indexed.
        let mut out_of_range = encoded;
        out_of_range[OBJ_NR_BYTE_INDEX] = 16;
        let error = project.update_from_sysex_response(&out_of_range);
        assert!(matches!(
            error,
            Err(RytmError::SysexConversion(
                SysexConversionError::InvalidObjNr
            ))
        ));
    }
}
