...
impl MacroParameter {
    ...
    /// Returns the parameter's stable short name, for example `amp_pan`.
...
impl SceneDefinitions {
    ...
    /// # Panics
    ///
    /// Never; every ID in `0..12` is a valid Scene ID.
    ...
    /// # Errors
    ///
    /// Scene IDs must be in `0..=11`, the lock target must be valid for its track, and the
    /// definition table must have a free record.
    ...
    /// # Errors
    ///
    /// Scene IDs must be in `0..=11`, every lock target must be valid for its track, and the
    /// definition table must have room for the replacement records.
    ...
    /// # Errors
    ///
    /// Both Scene IDs must be in `0..=11` and the definition table must have room for the copy.
...
impl PerformanceDefinitions {
    ...
    /// # Errors
    ///
    /// Performance IDs must be in `0..=11`.
    ...
    /// # Panics
    ///
    /// Never; every ID in `0..12` is a valid Performance ID.
    ...
    /// # Errors
    ///
    /// Performance IDs must be in `0..=11`, the lock target must be valid for its track, and the
    /// definition table must have a free record.
    ...
    /// # Errors
    ///
    /// Performance IDs must be in `0..=11`, every lock target must be valid for its track, and
    /// the definition table must have room for the replacement records.
    ...
    /// # Errors
    ///
    /// Both Performance IDs must be in `0..=11` and the definition table must have room for the
    /// copy.
    pub fn copy(&mut self, source: usize, target: usize) -> Result<(), RytmError> {
        copy_records(
            &mut self.raw,
...
const fn parameter_from_raw(track: MacroTrack, raw_id: u8) -> Option<MacroParameter> {
...
fn decode_performance_record(record: &[u8]) -> Option<PerformanceLock> {
    ...
    Some(PerformanceLock {
        track,
        parameter,
        depth: i8::from_ne_bytes([record[0]]),
    })
}
...
const fn macro_memory_full() -> RytmError {
    RytmError::SceneOrPerformanceMemoryFull
}
